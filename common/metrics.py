...
def compute_SS(preds, clusters, truncate, truncate_gt, reduce="mean"):
    ...
    for scanpath in preds:
        ...
        if is_fv:
            key = "test-{}-{}".format(
                scanpath["condition"], scanpath["name"].split(".")[0]
            )
        else:
            key = "test-{}-{}-{}".format(
                scanpath["condition"], scanpath["task"], scanpath["name"].split(".")[0]
            )
        ms = clusters[key]
        strings = ms["strings"]
        cluster = ms["cluster"]
...
def get_seq_score(preds, clusters, max_step, truncate_gt=False, tasks=None):
    ...
    if tasks is None:
        ...
        scores = []
        for task in tasks:
            scores.append(np.mean([r["score"] for r in results if r["task"] == task]))
...
def compute_SSS(
    preds, fixations, truncate, segmentation_map_dir, truncate_gt, reduce="mean"
):
    results = []
    for scanpath in preds:
        is_fv = scanpath["condition"] == "freeview"
        ...
        with gzip.GzipFile(
            join(segmentation_map_dir, scanpath["name"][:-3] + "npy.gz"), "r"
        ) as r:
...
def get_semantic_seq_score(
    preds, fixations, max_step, segmentation_map_dir, truncate_gt=False, tasks=None
):
    results = compute_SSS(preds, fixations, max_step, segmentation_map_dir, truncate_gt)
    if tasks is None:
        return np.mean([r["score"] for r in results])
    else:
        scores = []
...
def compute_sequence_KL(probs, gt_probs, patch_num):
    ...
    smooth_probs = (
        torch.from_numpy(smooth_probs)
        .view(smooth_probs.shape[0], -1)
        .to(gt_probs.device)
    )
    log_probs = torch.log(smooth_probs)
    kl_div = torch.nn.functional.kl_div(log_probs, gt_probs, reduction="batchmean")
    return kl_div
...
def compute_spatial_metrics_by_step(
    predicted_trajs, gt_scanpaths, im_w, im_h, prior_maps, end_step=1
):
    ...
    for sample_id in sample_ids:
        ...
        trajs = list(
            filter(lambda x: x["task"] == task and x["name"] == image, predicted_trajs)
        )
        ...
        pred_smap = utils.convert_fixations_to_map(fixs, im_w, im_h, smooth=True)

        gt_trajs = list(
            filter(lambda x: x["task"] == task and x["name"] == image, gt_scanpaths)
        )
        ...
        gt_smap = utils.convert_fixations_to_map(gt_fixs, im_w, im_h, smooth=True)
