...
class DCB_Env:
    ...
    def __init__(
        ...
        self.inhibit_return = ret_inhibition
    ...
    def observe(self, accumulate=True):
        ...
        if self.step_id > 0:
            ...
            lastest_fixation_on_feats = (
                self.fixations[:, self.step_id].to(dtype=torch.float32) / remap_ratio
            )
        ...
        return ext_states, self.fix_ind_map.clone()
    ...
    def step(self, act_batch):
        ...
        self.fix_ind_map[torch.arange(self.batch_size), self.step_id, py, px] = True

        # update action mask
        if self.inhibit_return:
            action_idx = torch.arange(
                0, self.batch_size, device=self.device, dtype=torch.long
            )
            if self.mask_size == 0:
                ...
                for i in range(bs):
                    self.action_mask[
                        i,
                        max(py[i] - self.mask_size, 0) : py[i] + self.mask_size + 1,
                        max(px[i] - self.mask_size, 0) : px[i] + self.mask_size + 1,
                    ] = 1
    ...
    def reset(self):
        ...
        self.fixations = torch.zeros(
            (self.batch_size, self.max_step, 2), dtype=torch.long, device=self.device
        )
        self.status = torch.zeros(
            self.batch_size, dtype=torch.uint8, device=self.device
        )
        self.is_active = torch.ones(
            self.batch_size, dtype=torch.uint8, device=self.device
        )
        ...
        self.fix_ind_map = self.init_fix_ind_map.clone()
        ...
        # center initialization
            ...
            if self.inhibit_return:
                ...
                self.action_mask[
                    ...
                    px - self.mask_size : px + self.mask_size + 1,
                ] = 1
                self.action_mask = self.action_mask.view(bs, -1)
    ...
    def set_data(self, data):
        ...
        self.init_fix_ind_map = data["fix_ind_map"].to(self.device)
        ...
        if self.inhibit_return:
            self.action_mask = (
                data["action_mask"].to(self.device).view(self.batch_size, -1)
            )
        else:
...
class CFI_Env:
    ...
    def observe(self, accumulate=True):
        ...
        if accumulate:
            ...
            fixs = self.fixations[:, self.step_id : self.step_id + 1].to(torch.float)
...
class FFN_Env:
    ...
    def observe(self):
        ...
        last_fixs = (
            self.fixations[:, self.step_id].unsqueeze(1).expand(-1, num_fixs_left, -1)
        )
        made_fixations = torch.cat(
            [self.fixations[:, : self.step_id + 1], last_fixs], axis=1
    ...
    def reset(self):
        ...
        # center initialization
            ...
            self.fixations[:, 0] = torch.tensor(
                [[int(self.pa.patch_num[0] / 2), int(self.pa.patch_num[1] / 2)]],
                dtype=torch.long,
                device=self.device,
            )
            bs = self.action_mask.size(0)
