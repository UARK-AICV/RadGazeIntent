...
def img_cortical_magnif_tsr(imgtsr, pnt, grid_func, demo=True):
    ...
    grid = (
        torch.stack(
            [
                (torch.tensor(XX_intp) / W * 2 - 1),  # normalize the value to -1, 1
                (torch.tensor(YY_intp) / H * 2 - 1),
    ...
    img_cm = F.grid_sample(
        imgtsr.unsqueeze(0), grid, mode="bilinear", padding_mode="zeros"
    )
    img_cm.squeeze_(0)
    if demo:
...
def cortical_magnif_tsr_demo(imgtsr, pnt, grid_func, subN=2):
    ...
    axs[1].scatter([pX], [pY], c="r", s=16, alpha=0.5)
    axs[2].scatter(
        XX_intp[::subN, ::subN].flatten(),
        YY_intp[::subN, ::subN].flatten(),
        c="r",
        s=0.25,
        alpha=0.2,
    )
...
def radial_quad_isotrop_gridfun(imgtsr, pnt, fov=20, K=20, cover_ratio=None):
    ...
    RadDistTfm = lambda R: (R < fov) * R + (R > fov) * (
        (R + K) ** 2 / 2 / (fov + K) + fov - (fov + K) / 2
    )
...
def radial_exp_isotrop_gridfun(imgtsr, pnt, slope_C=2.0, cover_ratio=None):
    ...
    RadDistTfm = (
        lambda R: 1 / slope * (np.exp(slope * R / np.sqrt(Hhalf**2 + Whalf**2)) - 1)
    )  # normalization
    ecc_tfm = RadDistTfm(
        ecc,
    )
...
if __name__ == "__main__":
    ...
    # %%
    img_cm = img_cortical_magnif_tsr(
        imgtsr, (80, 120), linear_separable_gridfun, demo=True
    )
    # %%
    ...
    img_cm = img_cortical_magnif_tsr(
        imgtsr,
        (100, 30),
        lambda img, pnt: radial_exp_isotrop_gridfun(
            img, pnt, slope_C=2.0, cover_ratio=0.4
        ),
        demo=True,
    )

    # %%  linear_separable
    rndMagnif = get_RandomMagnifTfm(
        grid_generator="radial_quad_isotrop", bdr=16, fov=20, K=0, cover_ratio=(0.05, 1)
    )
    ...
    # %%
    rndMagnif = get_RandomMagnifTfm(
        grid_generator="radial_exp_isotrop",
        bdr=64,
        slope_C=(0.75, 3.0),
        cover_ratio=(0.1, 0.5),
    )
