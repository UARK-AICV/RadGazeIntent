        ...
        while i <= len(data["saccade_x"]) - 1:
            ...
            if i == len(data["saccade_x"]) - 1:
                ...
                if data["saccade_rho"][i] < TAmp:
                    ...
                    if (data["fixation_dur"][-1] < TDur) or (
                        data["fixation_dur"][-2] < TDur
                    ):
            ...
            # if saccade is not the last one
                ...
                if (data["saccade_rho"][i] < TAmp) and (i < len(data["saccade_x"]) - 1):
                    ...
                    if (data["fixation_dur"][i + 1] < TDur) or (
                        data["fixation_dur"][i] < TDur
                    ):
        ...
        while i <= len(data["saccade_x"]) - 1:
            ...
            # elif the angle is small, but its the last saccade:
                ...
                if data["fixation_dur"][i + 1] < TDur:
                    ...
                    sim_len[j - 1] = rho
                    sim_dur.insert(
                        j, data["fixation_dur"][-1] + (data["fixation_dur"][i] / 2)
                    )
                    j -= 1
                    i += 1
                # if fixation duration is long:
            ...
            # else (the angle is too large
                ...
                # insert original data in new list -- no simplification
                sim_lenx, sim_leny, sim_x, sim_y, sim_theta, sim_len, sim_dur, i, j = (
                    keepsaccade(
                        i,
                        j,
                        sim_lenx,
...
def createdirectedgraph(szM, M, M_assignment):
    ...
    for i in range(0, len(adjacent)):
        weightedEdges.append(
            list(zip(list(adjacent.values())[i], list(weight.values())[i]))
        )
